...
/**
 * Allows to recover the quantities of materials purchased from a supplier and the expenditure incurred
 * 
 */
...
public class PurchasedMaterial {
	 ...
	 */
	private static final String SQL = "SELECT S.companyName AS companyName, M.name AS name, quantity, totalCost FROM (SELECT P.vat, P.productCode, SUM(P.quantity) AS quantity,  SUM(P.quantity*P.cost) AS totalCost FROM JobOrder J INNER JOIN Purchased P ON  P.id = J.id WHERE  '[2017-01-01,2019-12-31]'::daterange @> J.contractDate GROUP BY P.vat, P.productCode) AS G INNER JOIN Supplier S ON S.vat=G.vat INNER JOIN  Material M ON M.productCode = G.productCode ORDER BY S.companyName;";
